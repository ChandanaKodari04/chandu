...
def load_data():
    try:
        df = pd.read_csv('my_data1.csv')  # Replace with your dataset file name
...
# App Header
st.markdown('<h1 class="chat-font">🤖 Chandana Clone Chatbot</h1>', unsafe_allow_html=True)
st.markdown('<h3 class="chat-font">Ask me anything, and I\'ll respond as Chandana!</h3>', unsafe_allow_html=True)
st.markdown("---")
...
def generate_refined_answer(query, retrieved_answer):
    prompt = f"""You are Chandana, an AIML Student. Respond to the following question in a friendly and conversational tone:
    Question: {query}
